...
//    bobSessionBuilder.process(GROUP_SENDER, receivedAliceDistributionMessage);
    ...
    var ciphertextFromAlice =
        await aliceGroupCipher.encrypt(utf8.encode('smert ze smert'));
    try {
  ...
  test('testBasicRatchet', () async {
    ...
    var ciphertextFromAlice2 =
        await aliceGroupCipher.encrypt(utf8.encode('smert ze smert2'));
    var ciphertextFromAlice3 =
        await aliceGroupCipher.encrypt(utf8.encode('smert ze smert3'));
  ...
  test('testLateJoin', () async {
    ...
    var ciphertext =
        await aliceGroupCipher.encrypt(utf8.encode('welcome to the group'));
  ...
  test('testOutOfOrder', () async {
    ...
    for (var i = 0; i < 100; i++) {
      ciphertexts
          .add(await aliceGroupCipher.encrypt(utf8.encode('up the punks')));
    }
  ...
  test('testEncryptNoSession', () async {
    ...
    try {
      await aliceGroupCipher.encrypt(utf8.encode('up the punks'));
  ...
  test('testTooFarInFuture', () async {
    ...
    var tooFarCiphertext =
        await aliceGroupCipher.encrypt(utf8.encode('notta gonna worka'));
    try {
  ...
  test('testMessageKeyLimit', () async {
    ...
    for (var i = 0; i < 2010; i++) {
      inflight.add(await aliceGroupCipher.encrypt(utf8.encode('up the punks')));
    }
